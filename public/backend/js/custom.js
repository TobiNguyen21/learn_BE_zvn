$(document).ready(function () {
    ...
    //check selectbox
    change_form_action(".custom-select", "#zt-form", "#btn-action");
        ...
        success: function (res) {
            if (res.success) {
                ...
                if (span_tag.classList.contains('btn-success')) {
                    span_tag.classList.replace('btn-success', 'btn-warning');
                } else {
                    span_tag.classList.replace('btn-warning', 'btn-success');
                }
...
function handleOrderingChange(ad, id, ordering) {
    ...
    const input_tag = document.getElementById(`${id}_ordering`);
    ...
    if (!isNaN(newValue)) {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.success) {
                    input_tag.value = newValue;

                    // show toast 
                    Toast.fire({
                        icon: 'success',
                        title: 'change-ordering success. '
                    })
